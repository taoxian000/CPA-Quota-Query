...
程序从当前 Windows 用户的 `%USERPROFILE%\.codex\auth.json` 读取认证信息，并按 `auth_mode` 选择查询方式：`apikey`（以及兼容旧文件的未设置模式）沿用上面的 CPA 下游 API Key 接口；`chatgpt` 则读取 `tokens.access_token` 和 `tokens.account_id`，调用插件的 `/v0/resource/plugins/user-routing/quota/direct` 查询单个账户。ChatGPT 模式只支持查询，重置额度按钮不可用。两种方式都依赖 CPA 已加载 `CPA-Plugin-userrouting` 并启用了公开额度接口。
...
本地安装 Go 1.22 或更高版本后，运行 `build.bat`。脚本会将版本 `v1.2` 嵌入程序，并在当前目录生成 `quota-monitor-v1.2.exe`（Windows AMD64 GUI 程序，不会打开控制台窗口）。发布新版本时同步更新 `build.bat` 中的版本号。
